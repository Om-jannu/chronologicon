...
async function ingestionStatus(req, res) {
  ...
  const row = await jobsRepo.getJob(jobId);
  ...
  res.json({
    ...
    errors: (meta.errors || []).map(e => `Line ${e.lineNumber}: ${e.error}: '${e.line}'`),
...
async function getTimeline(req, res) {
  ...
  try {
    ...
    if (!timeline) return res.status(404).json({ message: "Event not found" });
    res.json(timeline);
  } catch (err) {
